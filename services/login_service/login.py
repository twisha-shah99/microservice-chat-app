...
def register():
    ...
    # Step 1: Call Profile service to create a new profile
    ...
    if profile_response.status_code != 200:
        # abort(400, description="Profile creation failed")
    ...
    if not profile_id:
        # abort(400, description="Invalid profile response")
        return make_response(jsonify({"error": "Invalid profile response"}), 400)

    # Step 2: Call Auth service to create an access token
...
def login():
    ...
    # Ensure both access_token and profile_id are provided
    if not access_token or not profile_id:
        abort(400, description="Both access_token and profile_id are required")

    # Step 1: Authenticate the access token by calling the Auth service
    ...
    # Step 2: Redirect to /chatrooms in the other service (running on port 8003)
